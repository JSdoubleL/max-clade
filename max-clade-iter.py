...
def node_to_tree(tree, node, up):
    ...
    Returns tuple (clade, remainder) both treeswift trees
    """
    parent = node.get_parent()
    parent.remove_child(node)
    tree.suppress_unifurcations()

    clade = tree.extract_subtree(node)
    clade.suppress_unifurcations()
    # c_tree = treeswift.read_tree_newick(tree.newick())
    # c_tree.reroot(node) 
    # t2 = c_tree.extract_subtree(parent)
    # t2.suppress_unifurcations()
    return (clade, tree) if up else (tree, clade)
    # if up:
    #     parent = node.get_parent()
...
def find_max_clades(tree, delimiter=None):
    ...
    max_clade, max_size, up = None, -1, False
    ...
    if not tree.root.dup_down:
        ...
        for clade in down_clades:
            if len(clade.down) > max_size:
                max_size = len(clade.down)
                max_clade = clade
        for clade in up_clades:
            if len(clade.up) > max_size:
                max_size = len(clade.up)
                max_clade = clade
                up = True
        # for clade in down_clades:
        ...
        if max_clade is None:
            return None, None

        clade_tree = tree.extract_subtree(max_clade)
        clade_tree.suppress_unifurcations()

        parent = max_clade.get_parent()
        parent.remove_child(max_clade)
        tree.suppress_unifurcations()
        
        return (clade_tree, tree) if not up else (tree, clade_tree)
...
def is_single_copy(tree, delimiter=None):
    """
    Detects if treeswift tree is a single-copy tree.

    Parameters
    ----------
    tree: treeswift tree
    delimiter: character separating species name from rest of leaf label

    Returns true if tree is single copy
    """
    return tree.num_nodes(internal=False) == len(set(l.label.split(delimiter)[0] for l in tree.traverse_postorder(internal=False)))
...
def main(args):
    ...
    with open(args.input, 'r') as fi, open(output, 'w') as fo:
        for line in tqdm(fi, total=num_trees):
            ...
            while True:
                unroot(tree)
                max_clade, remainder = find_max_clades(tree, args.delimiter)
                if max_clade is None:
                    break
                # get largest max-clade
                #max_clades.sort(key=lambda x:x[0].num_nodes(internal=False), reverse=True)
                #out.append(max_clades[0][0])
                out.append(max_clade)
                # delete from tree --- then iterate
                # if max_clades[0][1] is None:
                #     break
                if remainder is None:
                    break
                tree = treeswift.read_tree_newick(remainder.newick())
                #if tree is None: break
            #out.append(tree) # append remaining single-copy tree
